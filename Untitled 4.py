...
# Function to insert text with formatting into Google Docs
def insert_formatted_text(service, document_id, text, formatting):
	requests = [
		{
			'insertText': {
				'location': {
					'index': 1  # Insert at the start; adjust index as needed
				},
				'text': text
			}
		},
		{
			'updateTextStyle': {
				'range': {
					'startIndex': 1,  # Same as above index
					'endIndex': 1 + len(text)
				},
	...
	try:
		...
		print(f"Inserted and formatted text: '{text}'")
	except Exception as e:
		print(f"An error occurred while applying formatting: {e}")
		
# Function to construct and insert text based on row data
def construct_and_insert_text(service, document_id, row):
	descriptions = ['Description1', 'Description2', 'Description3', 'Description4', 'Description5']
	
	for desc in descriptions:
		if pd.notna(row[desc]):
			text = row[desc].replace('<total_cost>', f"£{row['Total Cost']:.2f}")
			formatting = FORMATTING[desc]
			insert_formatted_text(service, document_id, text, formatting)
...
template_document_id = '1Js0RYcdc-owkXmrl4ESKuMcMtxym0fmRZ2--bJYQr6Y'
...
def create_google_doc(template_document_id, form_data):
	...
	for prefix in prefixes:
		...
		# Assuming you want to insert combined_text somewhere in the document:
		# You might have a placeholder like <{prefix}_combined_text> in your document template.
		combined_text_placeholder = f"<{prefix}_combined_text>"
		doc_requests.append({'replaceAllText': {'containsText': {'text': combined_text_placeholder, 'matchCase': True}, 'replaceText': combined_text}})
		print(f"Inserted combined text for {prefix}.")
	...
	for index, row in data.iterrows():
		if row['Include'] == 'Y':
			construct_and_insert_text(docs_service, new_document_id, row)
