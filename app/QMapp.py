...
def builder_beta_page_editor(page_id):
	...
	if request.method == 'POST':
		warnings, selected_block_id = update_builder_beta_page_from_form(page_id, request.form)
		flash('Builder beta changes saved.', 'success')
		for warning in warnings:
			flash(warning, 'error')
...
def login():
	if request.method == 'POST':
		...
		# Admin login: username 'admin' + QM_ADMIN_PASSWORD
		...
		# User login: any non-empty username with password matching QM_USER_PASSWORD.
		# Replace with a real user store when multi-user auth is needed.
		if username and password == os.getenv('QM_USER_PASSWORD', 'user'):
			session['role'] = 'user'
