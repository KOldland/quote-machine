...
def apply_formatting_with_correct_indices(service, document_id, text_to_format, formatting):
	...
	# Log the document content and indices
	print(f"Document content: {document_content}")
	print(f"Formatting '{text_to_format}' from index {start_index} to {end_index}")
	
	# Check if indices are valid
	if start_index == -1 or end_index <= start_index:
		print(f"Text '{text_to_format}' not found or range is invalid.")
		return
...
# Function to generate combined text with cost and apply formatting
def generate_combined_text_with_cost(prefix, document_id, service):
	combined_text = []
	for index, row in data.iterrows():
		if row['Include'] == 'Y' and row['Line Code'].startswith(prefix):
			for col in ['Description1', 'Description2', 'Description3', 'Description4', 'Description5']:
				if pd.notna(row[col]):
					description = row[col]
					...
					# Apply formatting to each description
					if col in FORMATTING:
						apply_formatting_with_correct_indices(service, document_id, description, FORMATTING[col])
...
def create_google_doc(template_document_id, form_data):
	...
	# Calculate totals for specific prefixes and replace in the document
	...
	# Add replacement requests for combined texts
	combined_text_placeholders = ['<co>', '<si>', '<wp>', '<ab>', '<el>']
	for placeholder in combined_text_placeholders:
			prefix = placeholder.strip('<>')
			
			# Generate combined text with cost and apply formatting
			combined_text = generate_combined_text_with_cost(prefix, new_document_id, docs_service)
			
			doc_requests.append({'replaceAllText': {'containsText': {'text': placeholder, 'matchCase': True}, 'replaceText': combined_text}})
			print(f"Replaced {placeholder} with combined text.")
	...
	for index, row in data.iterrows():
		if row['Include'] == 'Y':
			...
			if descriptions:
				...
				# Added lines for logging startIndex and endIndex
				start_index = 1  # Added this line
				end_index = len(combined_description) + 1  # Added this line
				print(f"Formatting Description1: startIndex={start_index}, endIndex={end_index}")  # Added this line
				
				if 'Description1' in desc_col:
					doc_requests.append({
						'updateTextStyle': {
							'range': {
								'startIndex': start_index,  # Modified this line
								'endIndex': end_index  # Modified this line
							},
							'textStyle': FORMATTING['Description1'],
							'fields': 'fontSize,bold,foregroundColor'
						}
					})
				print(f"Replaced {placeholder} with combined description and applied formatting to Description1.")
	...
	# Apply simple formatting to test
	apply_simple_formatting(docs_service, new_document_id)	
