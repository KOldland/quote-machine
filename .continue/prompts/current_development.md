...
| 05/06/26 | UX Polish Sprint | **DONE**: Canvas padding added (`.builder-canvas-wrapper`). "Select Page" converted to collapsible `<details>/<summary>` with `▼` arrow. All emojis removed from Block Properties section headers. Username/role pill replaced with plain `user: <name> (<role>)` text. Sidebar set to `flex-column` so Log out button pushes to bottom via `margin-top: auto`. "Select Page" button alignment fixed by removing `.btn` class conflict — now uses `.btn-sidebar` only with `margin: 5px; width: calc(100% - 10px)` override. Changes in: `index.html`, `_builder_macros.html`, `main.css`. Commits: `e12a7d7`, `29cc581`, `5189622`, `e98d28f`. |
